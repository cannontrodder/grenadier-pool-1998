async (page) => {
  ...
  const runTuning = async (initial) => {
    ...
    await page.locator('#lock-distance').fill('60');
    await page.locator('#menu-close').click();
    const pull = await gesture(-Math.PI / 2, 70);
    o = await observe();
    check(!o.gesture.locked && !o.gesture.nearLocked, 'wide-radius pull leaves aim free');
    const initialAngle = o.aimAngle;
    await page.mouse.move(pull.end.x + pull.direction.y * 20, pull.end.y - pull.direction.x * 20);
    o = await observe();
    check(!o.gesture.locked && angleDistance(o.aimAngle, initialAngle) > .1, 'wide-radius sideways movement rotates aim');
    const lockedAngle = o.aimAngle;
    const ring = await page.locator('#precision').evaluate(e => {
      const m = e.getScreenCTM();
      return { radius: Number(e.getAttribute('r')) * Math.hypot(m.a, m.b), visible: getComputedStyle(e).display !== 'none' };
    });
    check(ring.visible && closeEnough(ring.radius, 60), 'precision circle shows fixed 60 CSS-pixel radius');
    const neutralRadius = await page.locator('#neutral').evaluate(e => Number(e.getAttribute('r')) * Math.hypot(e.getScreenCTM().a, e.getScreenCTM().b));
    check(closeEnough(neutralRadius, 36.9, 1), 'separate neutral circle still shows pull abort threshold');
    await page.mouse.move(pull.center.x - pull.direction.x * 45, pull.center.y - pull.direction.y * 45);
    o = await observe();
    check(o.gesture.nearLocked && o.gesture.locked && o.gesture.armed, 'inward approach protects angle while gentle shot remains armed');
    await page.mouse.move(pull.center.x - pull.direction.x * 42 + pull.direction.y * 8, pull.center.y - pull.direction.y * 42 - pull.direction.x * 8);
    o = await observe();
    check(angleDistance(o.aimAngle, lockedAngle) < 1e-9, 'movement inside precision circle cannot skew the angle');
    check(await page.locator('#aim').evaluate(e => e.classList.contains('locked')), 'near-white protection is visibly indicated');
    await screenshot('near-protected-guide');
    await page.mouse.up();
    o = await observe();
    check(o.phase === 'rolling' && o.shotId === 1 && angleDistance(o.aimAngle, lockedAngle) < 1e-9, 'gentle release preserves protected aim and shoots once');
    await reset();
    check((await observe()).tuning.guideLength === 100 && (await observe()).tuning.lockDistance === 60, 're-rack retains tuning');
    const nextPull = await gesture(-Math.PI / 2, 70);
    const outsideAngle = (await observe()).aimAngle;
    await page.mouse.move(nextPull.start.x, nextPull.start.y);
    o = await observe();
    check(o.gesture.nearLocked && !o.gesture.armed, 'neutral return protects angle and disarms');
    await page.mouse.move(nextPull.center.x - nextPull.direction.y * 18, nextPull.center.y + nextPull.direction.x * 18);
    o = await observe();
    check(angleDistance(o.aimAngle, outsideAngle) < 1e-9, 'tiny-radius rotation retains the displayed aim');
    await page.mouse.move(nextPull.center.x - nextPull.direction.y * 75, nextPull.center.y + nextPull.direction.x * 75);
    o = await observe();
    check(!o.gesture.locked && angleDistance(o.aimAngle, outsideAngle) > 1, 'moving outside precision circle resumes free aiming');
    await page.mouse.move(nextPull.start.x, nextPull.start.y);
    await page.mouse.up();
    check((await observe()).shotId === 0, 'inward release still aborts after re-aim');
    await page.locator('#menu-open').click();
    await page.locator('#lock-aim').uncheck();
    check(await page.locator('#lock-distance').isDisabled(), 'precision-radius control disabled when near-white protection is off');
    await page.locator('#menu-close').click();
