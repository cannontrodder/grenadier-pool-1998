# PROTOTYPE · T1 touch study · issue #7
...
Shot controls offers **A · Linear pull** and **B · Gentle start**. Both have the same physical arming threshold (18.9 CSS px past the starting radius); B squares the normalized pull for finer low power. Saturation is 127 px past the start. These are hypotheses, not accepted controls. No velocity-based mapping is implied.

Contact opens a separate selector; use the pad or keyboard sliders, and Reset to centre. Shot controls also offers angle/power sliders, Arm shot, Play shot and Abort. Arrows change angle, Shift+arrows change power, Escape aborts. Enter activates the focused control; Play shot must be explicitly armed.

“Lose turn in 3 seconds” closes the sheet and lets you start a gesture before the simulated interruption. Restore the turn from Shot controls, or Reset. Pointer cancellation, capture loss, blur, backgrounding, page exit and viewport/orientation change disarm; delayed release cannot shoot.

## Observation/action contract

Read-only `window.touchStudy.observe()` returns T1 version/full revision, deterministic scenario, animation frame and sample timestamp, phase (`ready`, `aiming`, `armed`, `rolling`), captured gesture, angle in radians, normalized power, mapping, contact point, readiness, turn ownership, shot count, last feedback, recent input events and each ball's world coordinates/velocities. Units: world table 1000×500, velocities world units/second. Portrait rotates that same world 90 degrees. `geometry()` returns cue/viewport screen coordinates; `health()` returns frame, revision/scenario and visibility.
...
The runner bounds CLI processes to 50 seconds, actions to 3.5 seconds, observation freshness to 2 seconds and settling to 7 seconds. Original failures retain the last 16 observations/actions, error, scenario/revision, completed checks, and a targeted screenshot under `output/playwright/touch-study/`. A command/session failure retains output and is a harness failure; violated observed behavior is a game failure. Timeout-only diagnoses remain harness uncertainty. Actual touch/browser evidence must be distinguished from synthetic interruption-event coverage and physical iPhone evidence.

## Fidelity and review boundary

Two circles, fixed-step rolling, exponential drag, simple equal-mass contact and cushion reflection; deterministic reset. Six correctly located pocket mouths are visual only. No potting, spin physics, sound, AI, persistence, rules, multiplayer, full match, 2v2, knockabout or 3D room. The contact selector records intent only. Reduced motion removes overlay transitions; motion of the balls remains the subject of the test. No camera animation. No final persona imagery is needed for this isolated gesture task; the linked design review retains it.
